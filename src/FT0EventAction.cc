...
//
//
/// \file FT0EventAction.cc
/// \brief Implementation of the FT0EventAction class
/// \brief Implementation of the FT0EventAction class
#include <iostream>
#include <fstream>
...
G4VHitsCollection* GetHC(const G4Event* event, G4int collId) {
  ...
  if (!hce) {
      ...
      G4Exception("FT0EventAction::EndOfEventAction()",
                  "FT0Code001", JustWarning, msg);
      return nullptr;
  }
...
void FT0EventAction::BeginOfEventAction(const G4Event*)
{
...
void FT0EventAction::EndOfEventAction(const G4Event* event)
{
  ...
  array<G4double, 1> totalCalEdep = {{ 0. }}; 
    ...
    for (unsigned long i = 0; i < hc->GetSize(); ++i) {
      G4double edep = 0.;
        ...
        auto hit = static_cast<FT0HadCalorimeterHit*>(hc->GetHit(i));   //check i & det to be correct!
